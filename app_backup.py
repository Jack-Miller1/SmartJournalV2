...
def create_journal():
    ...
    insights, tokens_used = ai_service.enhance_journal_entry(
        data.get('daily_summary', ''),
        data.get('journal_content', ''),
        data.get('mode', 'quick')
    )
    ...
    entry = JournalEntry(
        ...
        questions=json.dumps([]),  # Will be populated when generating questions
        answers=json.dumps([]),    # Will be populated when answering questions
...
def generate_questions():
    ...
    questions, tokens_used = ai_service.generate_reflection_questions(daily_summary, mode)
